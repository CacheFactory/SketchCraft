// skp2obj_win — Windows-portable version of skp2obj.
// Converts SketchUp .skp to Wavefront OBJ+MTL using the SketchUp C API.
// Compiled with MinGW for use under Wine on Linux (AWS Lambda).
// Usage: skp2obj_win.exe input.skp output.obj
...
// ─── Windows path helpers (replaces libgen.h) ─────────────────
...
// ─── API function declarations (resolved from SketchUpAPI.dll) ──

__declspec(dllimport) void SUInitialize(void);
__declspec(dllimport) void SUTerminate(void);

__declspec(dllimport) SUResult SUModelCreateFromFile(SUModelRef *model, const char *path);
__declspec(dllimport) SUResult SUModelRelease(SUModelRef *model);
__declspec(dllimport) SUResult SUModelGetEntities(SUModelRef model, SUEntitiesRef *entities);
__declspec(dllimport) SUResult SUModelGetNumMaterials(SUModelRef model, size_t *count);
__declspec(dllimport) SUResult SUModelGetMaterials(SUModelRef model, size_t len, SUMaterialRef *mats, size_t *count);

__declspec(dllimport) SUResult SUEntitiesGetNumFaces(SUEntitiesRef entities, size_t *count);
__declspec(dllimport) SUResult SUEntitiesGetFaces(SUEntitiesRef entities, size_t len, SUFaceRef *faces, size_t *count);
__declspec(dllimport) SUResult SUEntitiesGetNumEdges(SUEntitiesRef entities, int standalone_only, size_t *count);
__declspec(dllimport) SUResult SUEntitiesGetEdges(SUEntitiesRef entities, int standalone_only, size_t len, SUEdgeRef *edges, size_t *count);
__declspec(dllimport) SUResult SUEntitiesGetNumGroups(SUEntitiesRef entities, size_t *count);
__declspec(dllimport) SUResult SUEntitiesGetGroups(SUEntitiesRef entities, size_t len, SUGroupRef *groups, size_t *count);
__declspec(dllimport) SUResult SUEntitiesGetNumInstances(SUEntitiesRef entities, size_t *count);
__declspec(dllimport) SUResult SUEntitiesGetInstances(SUEntitiesRef entities, size_t len, SUComponentInstanceRef *instances, size_t *count);

__declspec(dllimport) SUResult SUFaceGetNumVertices(SUFaceRef face, size_t *count);
__declspec(dllimport) SUResult SUFaceGetVertices(SUFaceRef face, size_t len, SUVertexRef *vertices, size_t *count);
__declspec(dllimport) SUResult SUFaceGetOuterLoop(SUFaceRef face, SULoopRef *loop);
__declspec(dllimport) SUResult SUFaceGetNumInnerLoops(SUFaceRef face, size_t *count);
__declspec(dllimport) SUResult SUFaceGetInnerLoops(SUFaceRef face, size_t len, SULoopRef *loops, size_t *count);
__declspec(dllimport) SUResult SUFaceGetFrontMaterial(SUFaceRef face, SUMaterialRef *material);
__declspec(dllimport) SUResult SUFaceGetUVHelper(SUFaceRef face, int front, int back, SUTextureWriterRef tw, SUUVHelperRef *uvh);

__declspec(dllimport) SUResult SULoopGetNumVertices(SULoopRef loop, size_t *count);
__declspec(dllimport) SUResult SULoopGetVertices(SULoopRef loop, size_t len, SUVertexRef *vertices, size_t *count);

__declspec(dllimport) SUResult SUVertexGetPosition(SUVertexRef vertex, SUPoint3D *position);

__declspec(dllimport) SUResult SUEdgeGetStartVertex(SUEdgeRef edge, SUVertexRef *vertex);
__declspec(dllimport) SUResult SUEdgeGetEndVertex(SUEdgeRef edge, SUVertexRef *vertex);

__declspec(dllimport) SUResult SUGroupGetEntities(SUGroupRef group, SUEntitiesRef *entities);
__declspec(dllimport) SUComponentInstanceRef SUGroupToComponentInstance(SUGroupRef group);

__declspec(dllimport) SUResult SUComponentInstanceGetTransform(SUComponentInstanceRef instance, SUTransformation *transform);
__declspec(dllimport) SUResult SUComponentInstanceGetDefinition(SUComponentInstanceRef instance, SUComponentDefinitionRef *def);
__declspec(dllimport) SUResult SUComponentDefinitionGetEntities(SUComponentDefinitionRef def, SUEntitiesRef *entities);

__declspec(dllimport) SUResult SUMaterialGetName(SUMaterialRef material, SUStringRef *name);
__declspec(dllimport) SUResult SUMaterialGetColor(SUMaterialRef material, SUColor *color);
__declspec(dllimport) SUResult SUMaterialGetTexture(SUMaterialRef material, SUTextureRef *texture);
__declspec(dllimport) SUResult SUMaterialGetColorizeType(SUMaterialRef material, int *type);

__declspec(dllimport) SUResult SUTextureGetFileName(SUTextureRef texture, SUStringRef *file_name);
__declspec(dllimport) SUResult SUTextureWriteToFile(SUTextureRef texture, const char *file_path);

__declspec(dllimport) SUResult SUStringCreate(SUStringRef *out_string_ref);
__declspec(dllimport) SUResult SUStringRelease(SUStringRef *string_ref);
__declspec(dllimport) SUResult SUStringGetUTF8Length(SUStringRef string_ref, size_t *out_length);
__declspec(dllimport) SUResult SUStringGetUTF8(SUStringRef string_ref, size_t max_length, char *out_char_array, size_t *out_length);

__declspec(dllimport) SUResult SUUVHelperRelease(SUUVHelperRef *uvh);
__declspec(dllimport) SUResult SUUVHelperGetFrontUVQ(SUUVHelperRef uvh, SUPoint3D *point, SUUVQ *uvq);

__declspec(dllimport) SUResult SUTextureWriterCreate(SUTextureWriterRef *writer);
__declspec(dllimport) SUResult SUTextureWriterRelease(SUTextureWriterRef *writer);

// ─── Globals ────────────────────────────────────────────────────

static int g_vertex_offset = 1;
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        fprintf(stderr, "Usage: skp2obj_win.exe input.skp output.obj\n");
        return 1;
    }
